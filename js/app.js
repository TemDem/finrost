 ...
 */
    ...
    (() => {
        ...
        const iconMini = document.querySelector(".icon-menu_mini");
        const menuMini = document.querySelector(".menu__body_mini");
        if (iconMini && menuMini) iconMini.addEventListener("click", (function(e) {
            document.documentElement.classList.toggle("lock");
            document.documentElement.classList.toggle("menu-open_mini");
        }));
        document.addEventListener("click", (function(e) {
            if (!e.target.closest(".header")) if (document.documentElement.classList.contains("menu-open")) {
                document.documentElement.classList.remove("menu-open");
                document.documentElement.classList.remove("lock");
            }
            if (!e.target.closest(".header-mini")) if (document.documentElement.classList.contains("menu-open_mini")) {
                document.documentElement.classList.remove("menu-open_mini");
                document.documentElement.classList.remove("lock");
            }
        }));
        if (header && headerMini) window.addEventListener("scroll", (function(e) {
            if (header.getBoundingClientRect().top + 50 + header.getBoundingClientRect().height < 0) headerMini.classList.add("_active"); else if (headerMini.classList.contains("_active")) headerMini.classList.remove("_active");
        }));
        window["FLS"] = true;
        isWebp();
        menuInit();
